...
Things work a bit differently when interacting with **other smart contracts**. A smart contract can't initiate its own transfer, it can only move tokens that you've explicitly approved it to access. That's why ERC-20 includes the **approve → transferFrom** pattern:
