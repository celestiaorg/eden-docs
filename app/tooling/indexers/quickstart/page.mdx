...
This quickstart uses the RedStone multi-feed adapter as an example event source. The same Goldsky
workflow applies to other contracts: provide the contract address, ABI, network, and start block,
then deploy an instant subgraph.
...
<Callout type="info">
  Unlike the dedicated price feed contracts, the RedStone multi-feed adapter emits `ValueUpdate`
  events, making it directly indexable with an instant subgraph.
</Callout>
...
Using your deployed GraphQL endpoint (shown in the Goldsky dashboard or from `goldsky subgraph
list`), you can query the subgraph for data. Endpoint format:
