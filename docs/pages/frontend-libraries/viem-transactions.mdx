...
:::info
Eden testnet Blockscout indexes and displays type `0x76` transactions by hash. The explorer shows
the transaction status, block, executor, fee payer, calls, gas limit, and gas used. You can also
cross-check sponsored transactions with `cast tx`, `cast receipt`, or direct RPC calls.
:::
