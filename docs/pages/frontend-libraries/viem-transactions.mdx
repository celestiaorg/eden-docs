...
- Send sponsored transactions:
  - [`0x4776ece6dcfdaf35f7bc46e79ab3321055c73f761d8232afaa62565829f48721`](https://eden-testnet.blockscout.com/tx/0x4776ece6dcfdaf35f7bc46e79ab3321055c73f761d8232afaa62565829f48721)
  - [`0x2b6e28053be6423e05a957b954990cc35971b48eb20d280fea148357fb2d09c0`](https://eden-testnet.blockscout.com/tx/0x2b6e28053be6423e05a957b954990cc35971b48eb20d280fea148357fb2d09c0)
  - [`0x35310fd76c45f1441226c102f4dc1070b41ac66cb1e6ed3354da78aa69824a67`](https://eden-testnet.blockscout.com/tx/0x35310fd76c45f1441226c102f4dc1070b41ac66cb1e6ed3354da78aa69824a67)

  Eden RPC and `cast` show receipt status `1`, type `0x76`, an unfunded executor as `from`, and
  the sponsor as `feePayer`. The second sponsored run is included in canonical block `184343012`;
  the executor nonce advanced from `1` to `2`, the executor balance stayed `0`, and the sponsor paid
  `147000` wei of gas.

:::info
Eden testnet Blockscout indexes type `0x76` transactions by hash, but the web UI does not yet show
every custom field. Use `cast tx`, `cast receipt`, direct RPC calls, or the Blockscout transaction
API to verify the executor, fee payer, and full call list. The explorer's State tab may be slow or
show incomplete balance changes for sponsored transactions.
:::
...
Set an executor key, a funded sponsor key, and a target address. For this zero-data smoke test, use
an EOA as the target. If you use a contract, make sure it accepts empty calldata or replace `data`
with encoded calldata for a function it exposes.
