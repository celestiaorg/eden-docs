...
- [Deploy with the sponsored session key](https://eden-testnet.blockscout.com/tx/0xf2d386fd9f47b5916cf54aff957f98b6c0896d6fb06aad54c6c7d55203e9034d): Eden type `0x76`, with the session key as executor and the owner test account as `feePayer`.
...
This clears the stored session grant. It does not remove the EIP-7702 delegation from the EOA.
...
The verified sponsored transaction above reused the funded owner test account as the fee payer. The
commands below use a separate sponsor key, which is the shape you should use for app gas sponsorship.
